<template>
  <div class="verify-page">
    <div class="verify-box">
      <h2><el-icon><CircleCheck /></el-icon> 电子凭证验真</h2>
      <p class="sub">长江大学教务处签发的成绩证明单、证书、准考证、监考通知单等电子凭证，可在此核验真伪。凭证编号与验证码见凭证页脚。</p>
      <!-- Vue3 迁移：@submit.native.prevent → @submit.prevent；size="medium" → "default" -->
      <el-form :model="form" label-position="top" size="default" @submit.prevent>
        <el-form-item label="凭证编号">
          <el-input v-model="form.serialNo" placeholder="如 PG202609251200001234" clearable @keyup.enter="handleVerify" />
        </el-form-item>
        <el-form-item label="验证码">
          <el-input v-model="form.verifyCode" placeholder="凭证页脚 16 位验证码" clearable @keyup.enter="handleVerify" />
        </el-form-item>
...
h2 .el-icon { margin-right: 6px; vertical-align: middle; }
...
.result .el-icon { font-size: 18px; margin-right: 6px; vertical-align: middle; }
