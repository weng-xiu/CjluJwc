<template>
  <div class="public-search">
    <div class="container">
      <!-- 搜索框（Vue3 迁移：prefix-icon 字体类 → #prefix 插槽；slot="append" → #append；.native 移除） -->
        ...
        <div class="article-item" v-for="item in articles" :key="item.articleId"
          ...
          <div class="item-meta">
            <span v-if="item.columnName" class="meta-column">
              <el-icon><FolderOpened /></el-icon> {{ item.columnName }}
            </span>
      ...
      <!-- 分页（Vue3 迁移：:current-page.sync → v-model:current-page） -->
...
/* ========== 搜索高亮（v-html 内容需穿透） ========== */
.article-item :deep(.search-highlight) {
  color: #e65500;
  font-style: normal;
  font-weight: 600;
}
