<template>
  <div class="page-container">
    <el-card>
      <template #header><div class="card-header"><el-icon><Message /></el-icon> 教务通知</div></template>
      <el-table v-loading="loading" :data="noticeList" border stripe @row-click="handleDetail">
        ...
        <el-table-column label="类型" width="100" align="center">
          <template #default="scope"><el-tag size="small" :type="noticeTypeColor(scope.row.noticeType)">{{ noticeTypeText(scope.row.noticeType) }}</el-tag></template>
        </el-table-column>
    ...
    <!-- 通知详情：迁移 :visible.sync → v-model -->
    <el-dialog :title="currentNotice.noticeTitle" v-model="detailVisible" width="700px">
...
<style scoped>.page-container { max-width: 1200px; margin: 0 auto; }.card-header { font-size: 16px; font-weight: 600; display: flex; align-items: center; gap: 6px; }.notice-meta { color: #909399; font-size: 13px; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #ebeef5; }.notice-content { line-height: 1.8; min-height: 100px; }</style>
