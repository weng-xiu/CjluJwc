<template>
  <div class="page-container">
    <el-tabs v-model="activeTab" type="border-card" @tab-click="handleTabClick">
      ...
      <el-tab-pane label="我的毕业论文" name="mine" v-if="showMineTab">
        <div v-loading="mineLoading">
          ...
          <template v-if="!myThesis">
            ...
            <div class="toolbar">
              <el-input v-model="topicQuery.topicName" placeholder="按题目搜索" size="small" clearable style="width:220px" @keyup.enter="loadTopics" />
              <el-select v-model="topicQuery.topicSource" placeholder="题目来源" size="small" clearable style="width:150px" @change="loadTopics">
                <el-option v-for="item in topicSourceOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
              <el-input v-model="topicQuery.advisorName" placeholder="指导教师" size="small" clearable style="width:140px" @keyup.enter="loadTopics" />
              <el-button type="primary" size="small" icon="Search" @click="loadTopics">查询</el-button>
            </div>
            <el-table :data="topics" border stripe size="small" empty-text="暂无可选题目">
              ...
              <el-table-column label="来源" width="120" align="center">
                <template #default="scope">{{ topicSourceLabel(scope.row.topicSource) }}</template>
              </el-table-column>
              <el-table-column label="指导教师" width="100" align="center">
                <template #default="scope">{{ scope.row.advisorName || scope.row.advisor || '-' }}</template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="90" align="center">
                <template #default="scope">
                  <el-button link type="primary" size="small" :disabled="scope.row.electedCount >= scope.row.capacity"
          ...
          <template v-else>
            ...
            <el-descriptions :column="2" border size="small" style="margin:14px 0">
              ...
              <el-descriptions-item label="论文结论">
                <el-tag v-if="myThesis.isQualified === '1'" size="small" type="success">合格</el-tag>
                <el-tag v-else-if="myThesis.isQualified === '0'" size="small" type="danger">不合格</el-tag>
            ...
            <div class="sub-title">可执行操作
              <el-button v-if="canSubmitStage" type="primary" size="small" icon="Upload" style="margin-left:12px" @click="openSubmit">提交{{ stageLabel(myThesis.currentStage) }}材料</el-button>
              <el-button type="info" size="small" icon="View" @click="loadDegree">学位资格预审</el-button>
            </div>
            ...
            <el-timeline v-if="myProcesses.length">
              <el-timeline-item v-for="item in myProcesses" :key="item.processId" :timestamp="item.operateTime" :type="resultType(item.result)" placement="top">
                <div class="process-title">
                  <el-tag size="small" type="info">{{ stageLabel(item.stage) }}</el-tag>
                  <span class="process-action">{{ item.title || actionLabel(item.action) }}</span>
                  <el-tag size="small" :type="resultType(item.result)">{{ resultLabel(item.result) }}</el-tag>
            ...
            <template v-if="degree">
              ...
              <el-table :data="degreeItems" border stripe size="small">
                ...
                <el-table-column label="结论" width="90" align="center">
                  <template #default="scope"><el-tag :type="scope.row.ok ? 'success' : 'danger'" size="small">{{ scope.row.ok ? '达标' : '未达标' }}</el-tag></template>
                </el-table-column>
              </el-table>
      ...
      <el-tab-pane label="指导的论文" name="advisor" v-if="showAdvisorTab">
        <div v-loading="advisorLoading">
          <div class="toolbar">
            <el-input v-model="advQuery.planYear" placeholder="届别" size="small" clearable style="width:110px" @keyup.enter="loadAdvisor" />
            <el-input v-model="advQuery.studentName" placeholder="学生姓名" size="small" clearable style="width:130px" @keyup.enter="loadAdvisor" />
            <el-input v-model="advQuery.studentNo" placeholder="学号" size="small" clearable style="width:140px" @keyup.enter="loadAdvisor" />
            <el-select v-model="advQuery.currentStage" placeholder="当前环节" size="small" clearable style="width:130px">
              <el-option v-for="item in stageOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
            <el-select v-model="advQuery.stageStatus" placeholder="环节状态" size="small" clearable style="width:120px">
              <el-option v-for="item in stageStatusOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
            <el-button type="primary" size="small" icon="Search" @click="loadAdvisor">查询</el-button>
            <el-button size="small" icon="Refresh" @click="resetAdvisor">重置</el-button>
          </div>
          <el-table :data="advisorList" border stripe size="small" empty-text="暂无指导的论文">
            ...
            <el-table-column label="环节" width="170" align="center">
              <template #default="scope">
                <el-tag size="small" type="info">{{ stageLabel(scope.row.currentStage) }}</el-tag>
                <el-tag size="small" :type="statusTagType(scope.row.stageStatus)" style="margin-left:4px">{{ statusLabel(scope.row.stageStatus) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="查重" width="80" align="center">
              <template #default="scope">{{ scope.row.checkRate == null ? '-' : scope.row.checkRate + '%' }}</template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="230" align="center" fixed="right">
              <template #default="scope">
                <el-button link type="primary" size="small" @click="openAdvDetail(scope.row)">详情</el-button>
                <el-button link type="primary" size="small" v-if="scope.row.stageStatus === '1' && ['2','3','5'].indexOf(scope.row.currentStage) >= 0" @click="openAudit(scope.row)">审核</el-button>
                <el-button link type="primary" size="small" v-if="scope.row.currentStage === '4'" @click="openCheck(scope.row)">查重</el-button>
                <el-button link type="primary" size="small" v-if="scope.row.currentStage === '6' && !scope.row.archiveTime" @click="openArchive(scope.row)">归档</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination background layout="total, prev, pager, next" :total="advisorTotal"
                         :page-size="advQuery.pageSize" v-model:current-page="advQuery.pageNum"
    ...
    <!-- 学生提交环节材料 -->
    <el-dialog :title="'提交' + stageLabel(myThesis && myThesis.currentStage) + '材料'" v-model="submitOpen" width="560px" append-to-body>
    ...
    <!-- 教师环节审核 -->
    <el-dialog title="环节审核" v-model="auditOpen" width="540px" append-to-body>
      <el-form label-width="86px">
        <el-form-item label="学生">{{ auditRow.studentName }}（{{ auditRow.studentNo }}）</el-form-item>
        <el-form-item label="审核环节"><el-tag size="small">{{ stageLabel(auditForm.stage) }}</el-tag></el-form-item>
        <el-form-item label="审核结论">
          <el-radio-group v-model="auditForm.pass"><el-radio :value="true">通过</el-radio><el-radio :value="false">退回修改</el-radio></el-radio-group>
        </el-form-item>
    ...
    <!-- 教师查重登记 -->
    <el-dialog title="查重结果登记" v-model="checkOpen" width="540px" append-to-body>
      <el-form label-width="86px">
    ...
    <!-- 教师成绩归档 -->
    <el-dialog title="成绩归档" v-model="archiveOpen" width="540px" append-to-body>
      <el-form label-width="86px">
    ...
    <!-- 论文详情（教师） -->
    <el-dialog title="论文过程详情" v-model="advDetailOpen" width="720px" append-to-body>
...
export default {
  ...
  methods: {
    ...
    handleTabClick(tab) {
      // element-plus tab-click 回调参数为 TabsPaneContext，取 paneName
      if (tab.paneName === 'advisor' && this.advisorList.length === 0) this.loadAdvisor()
      if (tab.paneName === 'mine') this.loadMine()
    },
    /* ---------------- 学生侧 ---------------- */
    loadMine() {
...
.toolbar :deep(.el-input), .toolbar :deep(.el-select) { margin-right: 8px; }
