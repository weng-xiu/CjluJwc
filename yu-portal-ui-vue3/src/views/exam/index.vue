<template>
  <div class="page-container">
    <el-card>
      <template #header><div class="card-header"><el-icon><Tickets /></el-icon> 考试安排</div></template>
      ...
      <el-table v-loading="loading" :data="examList" border stripe>
        ...
        <el-table-column label="考试类型" prop="examType" width="90" align="center">
          <template #default="scope"><el-tag :type="scope.row.examType === 'final' ? 'primary' : 'warning'" size="small">{{ scope.row.examType === 'final' ? '期末' : '补考' }}</el-tag></template>
        </el-table-column>
