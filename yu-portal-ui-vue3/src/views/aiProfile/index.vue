    ...
    <el-card v-loading="loading">
      <template #header>
        <div class="card-header">
          <span><el-icon><DataAnalysis /></el-icon> 学业画像</span>
          <el-button link type="primary" size="small" icon="Refresh" @click="load">重新生成</el-button>
        </div>
      </template>
      ...
      <template v-else>
        ...
        <template v-if="(data.categoryScores || []).length">
          ...
          <el-table :data="data.categoryScores" border size="small">
            <el-table-column label="课程类别" min-width="140">
              <template #default="scope">{{ categoryName(scope.row.creditType) }}</template>
            </el-table-column>
        ...
        <el-table v-if="(data.warnings || []).length" :data="data.warnings" border size="small">
          ...
          <el-table-column label="等级" width="80" align="center">
            <template #default="scope">
              <el-tag :type="Number(scope.row.warningLevel) >= 2 ? 'danger' : 'warning'" size="small">{{ scope.row.warningLevel }} 级</el-tag>
            </template>
          </el-table-column>
        ...
        <div class="disclaimer" v-if="data.note"><el-icon><InfoFilled /></el-icon> {{ data.note }}</div>
        <div class="disclaimer"><el-icon><InfoFilled /></el-icon> 画像由本人成绩、培养方案、选课与预警数据实时计算，仅供学业自查参考，不作为毕业与学位授予依据。</div>
      </template>
    </el-card>
  </div>
</template>
