    ...
    <el-card v-loading="loading">
      <template #header>
        <div class="card-header">
          <span><el-icon><Finished /></el-icon> 毕业预审</span>
          <el-button type="primary" size="small" icon="Refresh" @click="load">重新测算</el-button>
        </div>
      </template>
      ...
      <el-table :data="data.items || []" border stripe size="small">
        ...
        <el-table-column label="结论" width="100" align="center">
          <template #default="scope"><el-tag :type="scope.row.qualified ? 'success' : 'danger'" size="small">{{ scope.row.qualified ? '达标' : '未达标' }}</el-tag></template>
        </el-table-column>
      </el-table>
      ...
      <template v-if="data.sections && data.sections.length">
        ...
        <el-table :data="data.sections" border stripe size="small">
          ...
          <el-table-column label="缺口" width="90" align="center"><template #default="scope"><span :class="{ warn: scope.row.gap > 0 }">{{ num(scope.row.gap) }}</span></template></el-table-column>
          <el-table-column label="结论" width="100" align="center">
            <template #default="scope"><el-tag :type="scope.row.qualified ? 'success' : 'danger'" size="small">{{ scope.row.qualified ? '达标' : '未达标' }}</el-tag></template>
          </el-table-column>
        </el-table>
      ...
      <div class="disclaimer" v-if="data.disclaimer"><el-icon><InfoFilled /></el-icon> {{ data.disclaimer }}</div>
    </el-card>
  </div>
</template>
<script>
