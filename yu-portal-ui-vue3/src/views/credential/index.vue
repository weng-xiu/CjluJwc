<template>
  <div class="page-container">
    <el-card>
      <template #header>
        <div class="card-header"><el-icon><Printer /></el-icon> 打印与电子凭证</div>
      </template>
        ...
        <el-tab-pane label="自助打印" name="print">
          ...
          <el-table v-if="bizType === 'CERTIFICATE'" v-loading="listLoading" :data="certList" border stripe size="small">
            ...
            <el-table-column label="证书类型" prop="certType" width="110">
              <template #default="scope">{{ certTypeText(scope.row.certType) }}</template>
            </el-table-column>
            <el-table-column label="发证日期" prop="certDate" width="120" />
            <el-table-column label="操作" width="180" align="center">
              <template #default="scope">
                <el-button link type="primary" size="small" @click="handleRowPreview('CERTIFICATE', scope.row.certId)">预览</el-button>
                <el-button link type="primary" size="small" @click="handleRowIssue('CERTIFICATE', scope.row.certId)" v-if="hasPermi('portal:credential:issue')">发放并打印</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <el-table v-if="bizType === 'EXAM_TICKET'" v-loading="listLoading" :data="seatList" border stripe size="small">
            ...
            <el-table-column label="操作" width="180" align="center">
              <template #default="scope">
                <el-button link type="primary" size="small" @click="handleRowPreview('EXAM_TICKET', scope.row.seatId)">预览</el-button>
                <el-button link type="primary" size="small" @click="handleRowIssue('EXAM_TICKET', scope.row.seatId)" v-if="hasPermi('portal:credential:issue')">发放并打印</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <el-table v-if="bizType === 'INVIGILATION'" v-loading="listLoading" :data="invigList" border stripe size="small">
            ...
            <el-table-column label="操作" width="180" align="center">
              <template #default="scope">
                <el-button link type="primary" size="small" @click="handleRowPreview('INVIGILATION', scope.row.invigilationId)">预览</el-button>
                <el-button link type="primary" size="small" @click="handleRowIssue('INVIGILATION', scope.row.invigilationId)" v-if="hasPermi('portal:credential:issue')">发放并打印</el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-tab-pane label="我的凭证" name="records">
          <el-table v-loading="recordLoading" :data="recordList" border stripe size="small">
            ...
            <el-table-column label="状态" width="80" align="center">
              <template #default="scope">
                <el-tag :type="scope.row.status === '0' ? 'success' : 'danger'" size="small">{{ scope.row.status === '0' ? '有效' : '已作废' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="90" align="center">
              <template #default="scope">
                <el-button link type="primary" size="small" @click="handlePrintRecord(scope.row)">打印</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <!-- 预览对话框 -->
    <el-dialog title="凭证预览" v-model="previewOpen" width="860px" append-to-body>
      <iframe :srcdoc="previewHtml" style="width: 100%; height: 520px; border: 1px solid #e4e7ed;"></iframe>
      <template #footer><el-button @click="previewOpen = false">关 闭</el-button></template>
    </el-dialog>
  </div>
</template>
