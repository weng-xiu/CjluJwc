  ...
  <div class="mobile-borrow">
    ...
    <div class="apply-block">
      <div class="block-title">
        <span>借用申请</span>
        <el-button type="primary" size="small" plain @click="applyOpen = !applyOpen">{{ applyOpen ? '收起' : '发起申请' }}</el-button>
      </div>
      <div v-show="applyOpen" class="apply-form">
        ...
        <div v-if="conflictTips.length" class="conflict-box">
          <div class="conflict-title"><el-icon><Warning /></el-icon> 检测到冲突：</div>
    ...
    <div v-else-if="list.length === 0" class="empty-state"><el-icon><OfficeBuilding /></el-icon><p>暂无借用申请</p></div>
    <div v-for="row in list" :key="row.borrowId" class="rec-card">
      ...
      <div class="rec-when"><el-icon><Calendar /></el-icon> {{ fmtDate(row.borrowDate) }} {{ row.startTime || '-' }}~{{ row.endTime || '-' }} · {{ row.attendeeCount || 0 }}人</div>
      ...
      <div class="rec-actions">
        <el-button v-if="row.procInstId" size="small" link type="primary" @click="openTrace(row)">流程进度</el-button>
...
.conflict-title .el-icon { vertical-align: middle; margin-right: 2px; }
...
.rec-when .el-icon { vertical-align: middle; margin-right: 2px; }
