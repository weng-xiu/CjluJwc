<template>
  <div class="mobile-schedule">
    <!-- 周次/日期切换（Vue3 迁移：字体图标 → el-icon 组件） -->
    <div class="week-bar">
    ...
    <div v-if="viewMode === 'week'" class="week-view">
      ...
      <!-- 课程网格（Vue3 迁移：filters 已移除，| truncate → truncate() 方法调用） -->
      <div class="schedule-grid">
        <div v-for="period in periods" :key="period.index" class="period-row">
          ...
          <div v-for="d in weekDays" :key="d.dayOfWeek" class="day-col period-cell">
            ...
            >
              <div class="card-name">{{ truncate(getCellCourse(d.dayOfWeek, period.index).courseName) }}</div>
      ...
      >
        ...
        <div class="card-body">
          ...
          <div class="card-bottom">
            <span class="card-info"><el-icon><User /></el-icon> {{ course.teacherName || '待定' }}</span>
            <span class="card-info"><el-icon><Location /></el-icon> {{ course.classroom || '待定' }}</span>
          </div>
        </div>
      </div>
    </div>

    <!-- 课程详情弹窗（Vue3 迁移：:visible.sync → v-model；span slot="footer" → template #footer） -->
    <el-dialog
    ...
    >
      <div class="detail-row"><el-icon><User /></el-icon> 教师：{{ detailCourse.teacherName || '待定' }}</div>
      <div class="detail-row"><el-icon><Location /></el-icon> 教室：{{ detailCourse.classroom || '待定' }}</div>
      <div class="detail-row"><el-icon><Clock /></el-icon> 时间：{{ detailCourse.startTime }} - {{ detailCourse.endTime }}</div>
      <div class="detail-row"><el-icon><Calendar /></el-icon> 周次：第 {{ detailCourse.weekNum || '-' }} 周</div>
      <template #footer>
...
export default {
  ...
  methods: {
    // Vue3 已移除 filters，原 `| truncate` 管道改为普通方法
    truncate(val) {
      if (!val) return ''
...
.week-label .el-icon { font-size: 12px; margin-left: 4px; color: #909399; }
...
.empty-state .el-icon { font-size: 48px; display: block; margin: 0 auto 12px; }
...
.card-info .el-icon { margin-right: 2px; vertical-align: middle; }
...
.detail-row .el-icon { color: #007ab8; margin-right: 6px; vertical-align: middle; }

/* 详情弹窗（Vue3 迁移：::v-deep → :deep()） */
:deep(.mobile-dialog) {
  border-radius: 12px;
}
</style>
