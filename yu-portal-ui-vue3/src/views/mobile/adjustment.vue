  ...
  <div class="mobile-adjust">
    ...
    <template v-if="tab === 'mine'">
      <div class="apply-entry">
        <el-button type="primary" size="small" class="submit-btn" @click="openApply"><el-icon><Plus /></el-icon> 发起调停课申请</el-button>
      </div>
      <div v-if="loading" class="loading-state"><el-icon class="is-loading"><Loading /></el-icon> 加载中...</div>
      <div v-else-if="list.length === 0" class="empty-state"><el-icon><Operation /></el-icon><p>暂无调停课申请</p></div>
    ...
    <!-- 待我审批 -->
    <template v-else>
      <div v-if="auditDenied" class="empty-state"><el-icon><Lock /></el-icon><p>{{ auditDenied }}</p></div>
      <div v-else-if="auditLoading" class="loading-state"><el-icon class="is-loading"><Loading /></el-icon> 加载中...</div>
      <div v-else-if="auditList.length === 0" class="empty-state"><el-icon><CircleCheck /></el-icon><p>暂无待审申请</p></div>
      <div v-for="row in auditList" :key="'a' + row.adjustId" class="rec-card">
        ...
        <div class="audit-btns">
          <el-input v-model="row._comment" size="small" placeholder="审批意见（选填）" class="audit-comment" />
          <div class="audit-ops">
    ...
    <div v-if="applyOpen" class="mask" @click.self="applyOpen = false">
      <div class="dialog">
        ...
        <div class="fi">
          <label>类型</label>
          <el-radio-group v-model="form.adjustType" size="small">
            <el-radio-button value="1">调课</el-radio-button>
            <el-radio-button value="2">停课</el-radio-button>
            <el-radio-button value="3">补课</el-radio-button>
          </el-radio-group>
        </div>
