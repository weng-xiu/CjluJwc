    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <el-icon><ChatDotRound /></el-icon> 教务政策智能问答
      ...
      <div ref="talkBox" class="talk-box" v-loading="loading">
        ...
        <div v-for="(msg, index) in messages" :key="index" :class="['msg', msg.role === 'user' ? 'msg-user' : 'msg-ai']">
          ...
          <template v-if="msg.role === 'ai'">
            <div class="msg-meta">
              <el-tag :type="sourceTag(msg.answerSource)" size="small">{{ sourceText(msg.answerSource) }}</el-tag>
            ...
            <div v-if="msg.errorMsg" class="msg-error"><el-icon><Warning /></el-icon> {{ msg.errorMsg }}</div>
            <div v-if="msg.references && msg.references.length" class="msg-refs">
              <div class="refs-title"><el-icon><Collection /></el-icon> 依据条文</div>
      ...
      <div v-if="suggests.length" class="suggest-box">
        ...
        <el-tag v-for="(item, index) in suggests" :key="index" size="small" class="suggest-tag"
                @click="ask(item)">{{ item }}</el-tag>
      </div>

      <!-- 输入区（Vue3 迁移：.native 修饰符移除，el-input 事件直接透传） -->
      <div class="input-box">
        <el-input
          v-model="question"
          type="textarea"
        ...
        <div class="input-actions">
          <el-button size="small" icon="RefreshLeft" @click="clearTalk">清空对话</el-button>
          <el-button type="primary" size="small" icon="Promotion" :loading="asking" @click="ask()">提问</el-button>
        </div>
      </div>
    </el-card>
  </div>
