<template>
  <div class="page-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span><el-icon><OfficeBuilding /></el-icon> 教室借用申请</span>
          <el-button type="primary" size="small" icon="Plus" @click="openApply">新建申请</el-button>
        </div>
      </template>
      ...
      <el-table v-loading="loading" :data="borrowList" border stripe>
        <el-table-column label="教室" min-width="150" show-overflow-tooltip>
          <template #default="scope">
            ...
            <el-tag v-if="scope.row.buildingName" size="small" type="info">{{ scope.row.buildingName }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="借用日期" width="110" align="center">
          <template #default="scope">{{ parseTime(scope.row.borrowDate, '{y}-{m}-{d}') }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="130" align="center">
          <template #default="scope">
            <el-button v-if="canCancel(scope.row)" link type="primary" size="small" @click="handleCancel(scope.row)">撤销</el-button>
            <el-button v-if="scope.row.procInstId" link type="primary" size="small" @click="handleTrace(scope.row)">进度</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- Vue3 迁移：el-pagination 的 :current-page.sync → v-model:current-page -->
    ...
    <!-- 新建申请 -->
    <el-dialog title="教室借用申请" v-model="applyOpen" width="560px" append-to-body>
      <el-form ref="applyForm" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="借用日期" prop="borrowDate">
          <!-- element-plus 用 dayjs：value-format yyyy-MM-dd → YYYY-MM-DD -->
