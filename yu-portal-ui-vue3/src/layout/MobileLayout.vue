<template>
  <div class="mobile-layout">
    ...
    <!-- 底部导航栏（Vue3 迁移：字体图标类 → el-icon + 动态组件，图标名见 data） -->
...
<script>
// Vue3 迁移：Vite 无 require，方法内 require('@/api/mobile') 改为顶部 ESM 导入
import { getWarningStatistics } from '@/api/mobile'
...
export default {
  ...
  data() {
    return {
      warningCount: 0,
      tabs: [
        { path: '/mobile', label: '首页', icon: 'HomeFilled' },
        { path: '/mobile/schedule', label: '课表', icon: 'Calendar' },
        { path: '/mobile/selection', label: '选课', icon: 'EditPen' },
        { path: '/mobile/grades', label: '成绩', icon: 'Document' },
        { path: '/mobile/warning', label: '预警', icon: 'Warning' }
      ]
    }
  },
  computed: {
