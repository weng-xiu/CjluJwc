<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="creditStructList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型编码" align="center" prop="creditType" />
      <el-table-column label="类型名称" align="center" prop="creditTypeName" />
      <el-table-column label="要求学分" align="center" prop="requiredCredit" />
      <el-table-column label="最低学分" align="center" prop="minCredit" />
      <el-table-column label="说明" align="center" prop="description" width="200" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="550px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="方案ID" prop="planId"><el-input v-model="form.planId" placeholder="请输入方案ID" /></el-form-item>
        <el-form-item label="类型编码" prop="creditType"><el-input v-model="form.creditType" placeholder="请输入学分类型编码" /></el-form-item>
        <el-form-item label="类型名称" prop="creditTypeName"><el-input v-model="form.creditTypeName" placeholder="请输入学分类型名称" /></el-form-item>
        <el-form-item label="要求学分" prop="requiredCredit"><el-input v-model="form.requiredCredit" placeholder="请输入要求学分" /></el-form-item>
        <el-form-item label="最低学分" prop="minCredit"><el-input v-model="form.minCredit" placeholder="请输入最低学分" /></el-form-item>
...
export default {
  name: "CreditStruct", dicts: ['sys_normal_disable'],
  data() { return { loading: true, ids: [], single: true, multiple: true, showSearch: true, total: 0, creditStructList: [], title: "", open: false,
    queryParams: { pageNum: 1, pageSize: 10, creditTypeName: null, status: null },
    form: {}, rules: { creditType: [{ required: true, message: "学分类型编码不能为空", trigger: "blur" }], creditTypeName: [{ required: true, message: "学分类型名称不能为空", trigger: "blur" }] } }
  },
  created() { this.getList() },
  methods: {
    getList() { this.loading = true; listCreditStruct(this.queryParams).then(response => { this.creditStructList = response.rows; this.total = response.total; this.loading = false }) },
    cancel() { this.open = false; this.reset() },
    reset() { this.form = { structId: null, planId: null, creditType: null, creditTypeName: null, requiredCredit: null, minCredit: null, description: null, status: "0" }; this.resetForm("form") },
    ...
    submitForm() { this.$refs["form"].validate(valid => { if (valid) { if (this.form.structId != null) { updateCreditStruct(this.form).then(response => { this.$modal.msgSuccess("修改成功"); this.open = false; this.getList() }) } else { addCreditStruct(this.form).then(response => { this.$modal.msgSuccess("新增成功"); this.open = false; this.getList() }) } } }) },
    handleDelete(row) { const structIds = row.structId || this.ids; this.$modal.confirm('是否确认删除学分结构编号为"' + structIds + '"的数据项？').then(function() { return delCreditStruct(structIds) }).then(() => { this.getList(); this.$modal.msgSuccess("删除成功") }).catch(() => {}) },
