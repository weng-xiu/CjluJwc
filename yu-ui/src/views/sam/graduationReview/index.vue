<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5"><el-button type="info" plain icon="el-icon-cpu" size="mini" @click="handleBatchReview" v-hasPermi="['sam:graduationReview:audit']">批量审核</el-button></el-col>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-cpu" @click="handleAutoReview(scope.row)" v-hasPermi="['sam:graduationReview:audit']">自动审核</el-button>
...
<script>
import { listGraduationReview, getGraduationReview, delGraduationReview, addGraduationReview, updateGraduationReview, autoReviewGraduation, batchReviewGraduation } from "@/api/sam/graduationReview"
export default {
  ...
  methods: {
    ...
    handleAutoReview(row) {
      this.$modal.confirm('是否对学生ID「' + row.studentId + '」执行自动审核？').then(() => {
        return autoReviewGraduation(row.studentId);
      }).then(response => {
    ...
    submitBatchReview() {
      ...
      batchReviewGraduation(ids).then(response => {
