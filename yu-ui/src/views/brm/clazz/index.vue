  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="所属专业" prop="majorId"><el-input v-model="form.majorId" placeholder="请输入所属专业ID" /></el-form-item>
...
export default {
  ...
  data() { return { loading: true, ids: [], single: true, multiple: true, showSearch: true, total: 0, clazzList: [], title: "", open: false,
    queryParams: { pageNum: 1, pageSize: 10, className: null, status: null },
    form: {}, rules: { classCode: [{ required: true, message: "班级编码不能为空", trigger: "blur" }], className: [{ required: true, message: "班级名称不能为空", trigger: "blur" }] } }
  },
  ...
  methods: {
    ...
    handleAdd() { this.reset(); this.open = true; this.title = "添加班级" },
    handleUpdate(row) { this.reset(); const classId = row.classId || this.ids; getClazz(classId).then(response => { this.form = response.data; this.open = true; this.title = "修改班级" }) },
