<template>
  <div class="page-container">
    <el-card>
      ...
      <el-table v-loading="loading" :data="adjustmentList" border stripe>
        ...
        <el-table-column label="原课表安排" min-width="150" align="center">
          <template slot-scope="scope">{{ scheduleText(scope.row.originalWeekDay, scope.row.originalStartPeriod, scope.row.originalEndPeriod) }}<span v-if="scope.row.originalClassroomName"> · {{ scope.row.originalClassroomName }}</span></template>
        </el-table-column>
        <el-table-column label="调整后安排" min-width="150" align="center">
          <template slot-scope="scope">{{ adjustTypeText(scope.row) }}</template>
        </el-table-column>
        ...
        <el-table-column label="审批状态" width="90" align="center">
          <template slot-scope="scope"><el-tag :type="statusColor(scope.row.approveStatus)" size="small">{{ statusText(scope.row.approveStatus) }}</el-tag></template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="80" align="center">
          <template slot-scope="scope">
            <el-button v-if="scope.row.approveStatus === '0'" size="mini" type="text" icon="el-icon-delete" style="color:#F56C6C" @click="handleCancel(scope.row)">撤销</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 新建申请 -->
    <el-dialog title="调停课申请" :visible.sync="dialogVisible" width="560px" :close-on-click-modal="false">
      <el-form ref="applyForm" :model="applyForm" label-width="110px" :rules="rules">
        ...
        <el-form-item label="申请类型" prop="adjustType">
          <el-radio-group v-model="applyForm.adjustType">
            <el-radio label="1">调课</el-radio><el-radio label="2">停课</el-radio><el-radio label="3">补课</el-radio>
          </el-radio-group>
        </el-form-item>
      ...
      <div slot="footer"><el-button @click="dialogVisible = false">取消</el-button><el-button type="primary" :loading="applying" @click="submitApply">提交申请</el-button></div>
    </el-dialog>
  </div>
</template>
<script>
