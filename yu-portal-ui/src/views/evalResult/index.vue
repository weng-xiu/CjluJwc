<template>
  <div class="page-container">
    ...
    <el-card class="mb16" v-loading="reportLoading">
      <div slot="header" class="card-header"><i class="el-icon-data-line"></i> 我的评教分析报告</div>
    ...
    <!-- 分课程结果 -->
    <el-card>
      <div slot="header" class="card-header"><i class="el-icon-notebook-2"></i> 分课程评教结果</div>
      <el-table v-loading="loading" :data="evalResults" border stripe>
        ...
        <el-table-column label="最高/最低" width="110" align="center">
          <template slot-scope="scope">{{ scope.row.maxScore }} / {{ scope.row.minScore }}</template>
        </el-table-column>
        <el-table-column label="满意度" width="100" align="center">
          <template slot-scope="scope"><el-tag :type="scope.row.satisfactionRate >= 85 ? 'success' : 'info'" size="mini">{{ scope.row.satisfactionRate }}%</el-tag></template>
        </el-table-column>
        <el-table-column label="操作" width="120" align="center" fixed="right">
          <template slot-scope="scope"><el-button type="primary" size="small" @click="viewDetail(scope.row)">查看评语</el-button></template>
        </el-table-column>
      </el-table>
      <el-empty v-if="!loading && !evalResults.length" description="暂无评教数据" />
    </el-card>

    <el-dialog title="课程评教评语" :visible.sync="detailVisible" width="700px">
