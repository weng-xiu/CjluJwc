  ...
  <div class="mobile-messages">
    ...
    <template v-if="tab === 'msg'">
      <div class="filter-bar">
        ...
        <el-button v-if="unread > 0" type="text" size="mini" class="read-all" @click="readAll">全部已读</el-button>
      </div>
    ...
    <template v-else>
      ...
      <div v-for="item in list" :key="item.todoId" class="todo-card">
        ...
        <el-button v-if="item.status === '0'" type="primary" size="mini" plain @click="finishTodo(item)">办结</el-button>
        <span v-else class="todo-done"><i class="el-icon-check"></i> 已办结</span>
      </div>
    </template>
