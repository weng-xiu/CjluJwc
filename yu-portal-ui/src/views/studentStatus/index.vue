        ...
        <el-tab-pane label="申请记录" name="records">
          <el-table :data="changeList" border stripe v-loading="recordLoading">
            <el-table-column label="异动类型" width="100" align="center">
              <template slot-scope="scope">{{ changeTypeText(scope.row.changeType) }}</template>
            </el-table-column>
            ...
            <el-table-column label="审批状态" width="100" align="center">
              <template slot-scope="scope"><el-tag :type="approveColor(scope.row.approveStatus)" size="small">{{ approveText(scope.row.approveStatus) }}</el-tag></template>
            </el-table-column>
            <el-table-column label="审批意见" prop="approveOpinion" min-width="140" show-overflow-tooltip />
            <el-table-column label="操作" width="140" align="center">
              <template slot-scope="scope">
                <el-button size="mini" type="text" icon="el-icon-view" @click="openTrace(scope.row)">进度</el-button>
...
export default {
  ...
  methods: {
    ...
    handleTabClick(tab) { if (tab.name === 'records') this.fetchRecords() },
    submitApply() {
