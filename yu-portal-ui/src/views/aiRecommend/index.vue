  ...
  <div class="page-container">
    <el-card v-loading="loading">
      <div slot="header" class="card-header">
        <i class="el-icon-magic-stick"></i> 个性化选课推荐
        <el-button type="text" size="mini" icon="el-icon-refresh" style="float:right" @click="load">刷新推荐</el-button>
      </div>
      ...
      <template v-else>
        ...
        <el-table :data="data.items || []" border stripe size="small" row-key="offeringId">
          <el-table-column type="expand">
            <template slot-scope="scope">
              <div class="signal-box">
                ...
                <el-table :data="scope.row.signals || []" size="mini" :show-header="false">
          ...
          <el-table-column label="课程名称" min-width="160" show-overflow-tooltip>
            <template slot-scope="scope">
              ...
              <el-tag v-if="scope.row.courseCategory" size="mini" type="info" style="margin-left:4px">{{ categoryName(scope.row.courseCategory) }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="容量" width="100" align="center">
            <template slot-scope="scope">{{ num(scope.row.enrolledCount) }}/{{ scope.row.maxStudents || '—' }}</template>
          </el-table-column>
          <el-table-column label="匹配度" width="120" align="center">
            <template slot-scope="scope">
              <el-tag :type="scoreTag(scope.row.score)" size="mini">{{ scope.row.score === null || scope.row.score === undefined ? '—' : scope.row.score }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="90" align="center" fixed="right">
            <template slot-scope="scope">
              <el-button type="primary" size="mini" :loading="enrollingId === scope.row.offeringId" @click="handleEnroll(scope.row)">选课</el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <div class="disclaimer" v-if="data.signalNote">
          <i class="el-icon-info"></i> 评分信号：{{ data.signalNote }}
        </div>
        <div class="disclaimer">
          <i class="el-icon-info"></i> 推荐结果仅依据培养方案、成绩、评教与容量等校内实数计算，不构成选课指令；正式选课仍以选课中心提交结果为准。
        </div>
      </template>
    </el-card>
  </div>
